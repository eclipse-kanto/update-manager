...
func TestOwnerConsentClientStart(t *testing.T) {
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			...
			mockPaho.EXPECT().Subscribe(test.domain+"update/ownerconsentfeedback", uint8(1), gomock.Any()).Return(mockToken)
...
func TestOwnerConsentClientStop(t *testing.T) {
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			...
			mockPaho.EXPECT().Unsubscribe(test.domain + "update/ownerconsentfeedback").Return(mockToken)
...
func TestSendOwnerConsent(t *testing.T) {
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			...
			assertOutgoingResult(t, test.isTimedOut, client.SendOwnerConsent(name, testConsent))
		})
	}
}

func TestHandleOwnerConsentFeedback(t *testing.T) {
	tests := map[string]testCaseIncoming{
		"test_handle_owner_consent_feedback_ok":         {domain: "testdomain", handlerError: nil, expectedJSONErr: false},
		"test_handle_owner_consent_feedback_error":      {domain: "mydomain", handlerError: errors.New("handler error"), expectedJSONErr: false},
		"test_handle_owner_consent_feedback_json_error": {domain: "testdomain", handlerError: nil, expectedJSONErr: true},
	}
	...
	testConsent := &types.OwnerConsentFeedback{
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			...
			handler.EXPECT().HandleOwnerConsentFeedback(name, gomock.Any(), testConsent).Times(expectedCalls).Return(test.handlerError)
			...
			mockMessage.EXPECT().Topic().Return(test.domain + "update/ownerconsentfeedback")
